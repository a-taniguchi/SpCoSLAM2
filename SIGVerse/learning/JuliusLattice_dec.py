...
# 実行すれば、自動的に指定フォルダ内にある音声ファイルを読み込み、Juliusでラティス認識した結果を出力してくれる。（GMM、DNN両方切り替え対応版）
...
def RecogLattice( wavfile , step , filename , trialname):
    ...
    if (JuliusVer == "v4.4" and HMMtype == "DNN"):
      if (step == 0 or step == 1):  #最初は日本語音節のみの単語辞書を使用(step==1が最初)
        p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-dnn.jconf -v " + lmfolder + lang_init + " -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -lattice -dnnconf " + Juliusfolder + "julius.dnnconf $*" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS -confnet 
        print "Julius",JuliusVer,HMMtype,"Read dic:" ,lang_init , step
      else:  #更新した単語辞書を使用
        p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-dnn.jconf -v " + datafolder + trialname + "/" + str(max(step-(LMLAG2 == 0)-LMLAG2,1)) + "/WD.htkdic -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -lattice -dnnconf " + Juliusfolder + "julius.dnnconf $*" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS  -confnet
        print "Julius",JuliusVer,HMMtype,"Read dic: " + str(max(step-(LMLAG2 == 0)-LMLAG2,1)) + "/WD.htkdic" , step
    else:
      if (step == 0 or step == 1):  #最初は日本語音節のみの単語辞書を使用(step==1が最初)
        p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + lmfolder + lang_init + " -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -lattice" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS -confnet 
        print "Julius",JuliusVer,HMMtype,"Read dic:" ,lang_init , step
      else:  #更新した単語辞書を使用
        p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + datafolder + trialname + "/" + str(max(step-(LMLAG2 == 0)-LMLAG2,1)) + "/WD.htkdic -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -lattice" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS  -confnet
        print "Julius",JuliusVer,HMMtype,"Read dic: " + str(max(step-(LMLAG2 == 0)-LMLAG2,1)) + "/WD.htkdic" , step
