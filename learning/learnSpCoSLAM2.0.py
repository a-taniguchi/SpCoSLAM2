...
##############################################
# SpCoSLAM 2.0 online learning program (無駄なコメントアウトコードを省いたバージョン+bugfix)
# かつ、SpCoAのオンライン版（SpCoSLAMから言語モデル更新と画像特徴を除けるバージョン）
# latticelmを並列化するバージョン（確認済み）
# 2.0系の追加要素を導入、切り替え可能にする予定のバージョン（実装中）
# Akira Taniguchi 2017/01/18-2017/02/02-2017/02/15-2017/02/21-2018/02/16-
##############################################

#注意：パーティクル番号の対応関係が間違っている可能性がある（評価用の方と合わせて確認が必要）

##########---遂行タスク2.0---##########

#評価用プログラム：Juliusの音声認識のCPU並列化orGPU使用（仮想では使えない）
